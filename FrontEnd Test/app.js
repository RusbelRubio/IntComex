...
const _EXTERNAL_URL = 'https://localhost:7124/api/Customers';
//Configure Client 
...
async function getDataToApi() {
    ...
    let config = {
      ...
      await instanceAxios(config)
      .then(function (response) {        
        //dataFound=JSON.stringify(response.data);  
        dataFound=response.data;  
        console.log('Valores obtenidos Get:'+ dataFound);        
      })
      .catch(function (error) {
...
// Listen on Port 3000
app.listen(3000, ()=>{
    console.log('Test Development Intcomex: SERVER UP en http://localhost:3000')
})